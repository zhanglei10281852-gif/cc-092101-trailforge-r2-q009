...
报名、退订、活动编辑和状态变更属于“先检查后写入”事务：SQLite 默认的延迟事务在第一条写语句前不持有任何锁，容量、行程互斥、重复报名和候补顺序的检查与写入之间会被其他连接插入。这些事务在开始时通过 `BEGIN IMMEDIATE` 立即取得数据库写锁，把同一数据库文件上的写事务（包括其他进程）串行化，不依赖任何进程内锁。等待写锁超过 busy timeout 时返回 503 `database_busy`，与容量不足、重复报名、行程冲突、幂等键冲突等 409 业务错误严格区分；`Database.write_session` 为其他写路径提供同样的语义。

训练计划、训练记录、活动、报名、装备借还、风险和签到等关键变更都会写结构化审计日志。日志包含操作者、UTC 时间、对象、动作、前后状态和必要上下文；审计工具会过滤密码、令牌、密钥等敏感字段。报名、幂等记录和审计日志在同一个事务中提交，失败时整体回滚，不会留下残片。
